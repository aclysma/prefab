    ...
    pub fn create_transaction_diffs(
        ...
        for (entity_uuid, entity_info) in &self.uuid_to_entities {
            ...
            for (component_type, registration) in registered_components {
                ...
                let mut ser = bincode::Serializer::new(&mut apply_data, bincode::config::DefaultOptions::new());
                let mut ser_erased = erased_serde::Serializer::erase(&mut ser);
                ...
                if apply_result != DiffSingleResult::NoChange {
                    ...
                    let mut ser = bincode::Serializer::new(&mut revert_data, bincode::config::DefaultOptions::new());
                    let mut ser_erased = erased_serde::Serializer::erase(&mut ser);
